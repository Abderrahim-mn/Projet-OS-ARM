...
static int dev_init_leds(Device *dev)
{
	...
	/* A COMPLETER */
	dev->mutex = sem_new(1);//create mutex
...
{
    dev->mutex = sem_new(1);
    dev->sem_read = sem_new(0);  // Sémaphore de lecture
    dev->sem_write = sem_new(1); // Sémaphore d'écriture
    return dev->mutex && dev->sem_read && dev->sem_write ? 1 : 0;
}
...
static int dev_read_ser(FileObject *f, void *buf, size_t len)
{
    ...
    // Lecture de données USART en mode bloquant
    USART_ReadBlocking(USART0, (uint8_t *)(buff + f->offset), len);
    cCount = len;
...
static int dev_write_ser(FileObject *f, void *buf, size_t len)
{
    sem_p(f->dev->mutex);  // Entrée dans la section critique

    // Écriture de données USART en mode bloquant
    USART_WriteBlocking(USART0, (uint8_t *)buf, len);

    sem_v(f->dev->mutex);  // Sortie de la section critique
